...
### Predictability (可预测性)
Skill 使 Agent 在每次运行中采取相同的物理执行过程。最高美德。
- *Avoid*: 一致性、可靠性、输出确定性
...
### Description (描述)
Skill 的机器可读触发器。模型调用的常驻 Context Pointer。删去即转为 User-Invoked。

### Context Pointer (上下文指针)
常驻 context 中的物理引用（如 Markdown 链接）。决定资料加载时机与可靠性。

### Context Load (上下文负载)
模型调用 `description` 逐轮占用 context 的 Token 与注意力物理开销。

### Cognitive Load (认知负载)
人类记住 skill 存在与触发时机的记忆负担。

### Router Skill (路由器技能)
索引其他 User-Invoked skill 的入口技能。降低 Cognitive Load。

### Granularity (粒度)
Skill 拆分细度。By Invocation 按引导词切分；By Sequence 隐去后置步骤防止 Premature Completion。
...
Skill 系统之外的静态文件，无 description，不可独立调用。
...
### Sprawl (膨胀)
*失败模式*。Skill 体积仅因物理字数过长而损坏可读性。解法：信息层级下沉。
...
当前步骤之后的后续步骤。露在 context 中诱导 Premature Completion。
