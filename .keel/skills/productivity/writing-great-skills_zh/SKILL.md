---
name: writing-great-skills_zh
description: 编写与编辑高确定性 AI Skill 的规范指南。中文审阅版。
...
- **Model-invoked**：省略 `disable-model-invocation`。描述在每轮对话中常驻 context（产生 Context Load）。用于 Agent/Skill 自主触发。
- **User-invoked**：配置 `disable-model-invocation: true`。对 Agent 物理隐藏（零 Context Load）。仅允许人类在终端显式输入 `/command` 触发。
- **Router Skill**：当用户调用 skill 过多时，创建一个 User-invoked skill 作为入口，统一索引其他 skill。

## 2. 描述编写规范 (Description)

- **前置核心词**：将 description 的核心触发词放在句首。
- **一分支一触发**：每个逻辑分支只保留 1 个核心触发词，擦除所有同义词重构。
- **禁止重复正文**：description 仅声明触发条件，禁止重复正文中的实现细节。
...
- **按调用切分**：当某个功能有独立触发词，或需要被其他 skill 调用时，切出独立的 Model-invoked skill。
- **按顺序切分**：当后续步骤会干扰 Agent 当前步骤的专注度时，将后续步骤隐藏到独立 skill 中（防止 Premature Completion）。

## 5. 剪裁与无用语句擦除 (Pruning)

- **Single Source of Truth**：每个规则只在一处定义，禁止多处重复。
- **No-op Test**：句级测试。如果删去某句话对 Agent 的行为零影响，直接整句擦除。
- **Leading Words**：使用预训练固有词汇（如 `tight`, `red`, `tracer bullet`）替代长篇解释。
...
- **Sprawl**：skill 文件过长。下沉参考资料到独立文件。
- **No-op**：默认已遵循的冗余规则。整句擦除。
- **Negation**：使用否定句反向激活错误行为。强制使用 Positive Assertions 规定目标行为。
