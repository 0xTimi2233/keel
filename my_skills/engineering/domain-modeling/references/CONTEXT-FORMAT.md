...
客户下单后产生的交易记录。
等价: Order
避免: Purchase, transaction

**发票**:
货物交付后发给客户的付款请求。
等价: Invoice
避免: Bill, payment request

**客户**:
下订单的个人或组织。
等价: Customer
避免: Client, buyer, account
```

## 规则

- **有主见**：同一概念有多个词时，选定最好的一个，其余列入 `避免`
- **中英文等价**：主词使用项目日常语言，中文项目用中文术语；同一概念在代码或文档中的名称列入 `等价` 字段
- **定义紧凑**：最多一两句话，定义它**是什么**，而不是它做什么
- **只收本上下文特有的术语**：通用编程概念（超时、错误类型、工具模式）不属于这里，即使项目大量使用。添加前先判断：这是该上下文独有的概念，还是通用编程概念？只有前者才收录
