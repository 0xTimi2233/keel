...
| 项 | 命令 | 请求体 |
|---|---|---|
| secret scanning | `gh api --method PUT repos/{owner}/{repo}/secret-scanning --input -` | {"enabled": true} |
...
| 依赖安全更新 | `gh api --method PUT repos/{owner}/{repo}/automated-security-fixes --input -` | {"enabled": true} |

push protection 状态查询：`gh api repos/{owner}/{repo} --jq '.security_and_analysis.secret_scanning_push_protection.status'`

## 权限

分支保护与安全设置需要仓库 Admin 权限，检查：`gh api repos/{owner}/{repo} --jq '.permissions.admin'`，非 true 时报告"需要 Admin 权限"并停止
